...
tableextension 75439 ExtReservationEntrySave extends "Reservation Entry Save"
{
    fields
    {
    ...
    local procedure FilterLinesWithItemToPlan(var Item: Record Item; IsReceipt: Boolean)
    begin
        Reset;
        SetCurrentkey(
    ...
    local procedure FindLinesWithItemToPlan(var Item: Record Item; IsReceipt: Boolean): Boolean
    begin
        FilterLinesWithItemToPlan(Item, IsReceipt);
        exit(Find('-'));
    end;

    local procedure LinesWithItemToPlanExist(var Item: Record Item; IsReceipt: Boolean): Boolean
    begin
        FilterLinesWithItemToPlan(Item, IsReceipt);
        exit(not IsEmpty);
    end;
    ...
    var
        ReservationEntrySave_lRec: Record "Reservation Entry Save";
    begin
        //C0009-NS 241018
        ReservationEntrySave_lRec.Reset;
        if ReservationEntrySave_lRec.FindLast then
            exit(ReservationEntrySave_lRec."Entry No." + 1)
        else
            exit(1);
        //C0009-NE 241018
    end;
